...
def main(config,
    ...
    # dom size
    dom_size = [16, 16]
    # Number of Test Scenario
    num_scene = 7
    for dom in range(n_domains):
        # Randomly select goal position
        # find goal
        goal_path = 'data\\scenarios_grid\\Scenario_goal' + str(num_scene) + '.png'
        ...
        grid_path = 'data\\scenarios_grid\\Scenario_' + str(num_scene) + '_grid.png'
        ...
        mat_path = 'data\\scenario_states\\Gen' + str(num_scene) + '.mat'
        Scenario = io.loadmat(mat_path)
        X = np.array(Scenario['X'])
        Y = np.array(Scenario['Y'])
        FPA = np.array(Scenario['gamma'])
        ...
        for i in range(n_traj):
            if len(X) > 1:
                ...
                pred_traj = np.zeros((L, 3))
                # Set starting position
                pred_traj[0, :] = [X[0], Y[0], FPA[0]]
                ...
                for j in range(1, L):
                    ...
                    im_data = 1 - im_data
                    # im_data = im_data.reshape(1, 1, dom_size[0], dom_size[1])
                    ...
                    S2_in = torch.from_numpy(state_data[1].reshape(
                        [1, 1]))
                    gamma_in = torch.from_numpy(state_data[2].reshape(
                        [1, 1]))
                    ...
                    if use_GPU:
                        ...
                        S1_in = S1_in.cuda()
                        S2_in = S2_in.cuda()
                        gamma_in = gamma_in.cuda()
                    # Wrap to autograd.Variable
                    X_in, S1_in, S2_in, gamma_in = Variable(X_in), Variable(S1_in), Variable(S2_in), Variable(gamma_in)
                    #if j==110:
                    #   break
                    # Forward pass in our neural net
                    a = vin(X_in, S1_in, S2_in,gamma_in, config)
                    # Transform prediction to indices
                    s = np.array([pred_traj[j - 1, 0], pred_traj[j - 1, 1]])
                    gamma_now = np.array(pred_traj[j - 1, 2])
                    # print(s)
                    # print("action :" + str(a))
                    # print("gamma :" + str(gamma_now))
                    ns, gamma = get_next_states(s, gamma_now, a)
                    nx = ns[0]
                    ny = ns[1]
                    pred_traj[j, 0] = nx
                    pred_traj[j, 1] = ny
                ...
                total += 1
                if config.plot == True:
                    visualize(im_obs, np.concatenate((X, Y), axis=1), pred_traj)
...
def visualize(dom, states_xy, pred_traj):
    fig, ax = plt.subplots()
    plt.imshow(dom, cmap="Greys_r")#, extent = [-50, 50, -50, 50])
    ax.plot(states_xy[:, 0], states_xy[:, 1], c='b', label='Optimal Path')
    ax.plot(
        pred_traj[:, 0], pred_traj[:, 1], '-', c='r', label='Predicted Path')
...
def get_next_states(s, gamma, a):
    ...
    y = s[1]
    vel = 0.04
    x = x + vel * math.cos(gamma) * dT
    y = y + vel * math.sin(gamma) * dT

    gamma = gamma.item(0) + (a / 0.25) * dT
    if gamma>2*math.pi:
